...
def filter_zero_xyz_rows(df: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    ----------
    df : pd.DataFrame
        Input DataFrame with columns 'x', 'y', 'z'.

    Returns
    -------
    pd.DataFrame
        DataFrame with all-zero rows removed.
    """
    mask = (df['x'] == 0) & (df['y'] == 0) & (df['z'] == 0)
    return df[~mask]


def remove_zero_xyz_rows() -> None:
    """
    ...
    logger.info("Removed %d zero-rows from raw data.", removed)
    logger.info("Saved cleaned output: %s", output_csv)
